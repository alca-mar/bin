...
PATH_fotos_originales="~/Imágenes/FotosOriginales"
...
if [ "$fotos_u" = '1' ]
 then
 RA_fotos="$PATH_carpeta"
elif [ "$fotos_u" = '2' ]
 then
 RA_fotos="$PATH_fotos2"
elif [ "$fotos_u" = '3' ]
then
...
# Solo proceso archivos modificables (writeable)
 if [ -w "$file" ]; then
   ...
   cp "$file" "$PATH_fotos_originales/$nombre_foto_original"
